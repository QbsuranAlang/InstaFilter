	...
	else if(img->nChannels != 1)
	{
		for(int i = 0 ; i < img->height ; i++)
			for(int j = 0 ; j < img->widthStep ; j+=img->nChannels)
			{
				...
				double newB = (B - G - R) * 3 / 2;
				double newG = (G - R - B) * 3 / 2;
				double newR = (R - G - B) * 3 / 2;

				if(newB < 0) newB = -newB; else if(newB > 255) newB = 255;
				if(newG < 0) newG = -newG; else if(newG > 255) newG = 255;
				if(newR < 0) newR = -newR; else if(newR > 255) newR = 255;
