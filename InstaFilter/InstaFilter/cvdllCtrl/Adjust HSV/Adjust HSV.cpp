...
IplImage *img, *dst;
static char *windowName = "調整HSV";
static char *ctrlPanel = "控制面板";
int HuePosition = 180;
int SaturationPosition = 100;
int ValuePosition = 100;
...
{
	float Hue = HuePosition - 180;
	float Saturation = (SaturationPosition - 100) * 2.55; //200->255
	float Value = (ValuePosition - 100)/100.;

	//create float image
	IplImage *temp = cvCreateImage(cvGetSize(img), IPL_DEPTH_32F, img->nChannels);
	cvConvertScale(img, temp, 1.0/255.0, 0);
	//split 
	IplImage* floatingH = cvCreateImage( cvGetSize(img), IPL_DEPTH_32F, 1 );
	IplImage* floatingS = cvCreateImage( cvGetSize(img), IPL_DEPTH_32F, 1 );
	IplImage* floatingV = cvCreateImage( cvGetSize(img), IPL_DEPTH_32F, 1 );
	...
	//process saturation
	cvConvertScale( temp, dst, 255.0, 0 );

	IplImage *hsv = convertImageRGBtoHSV(dst);
...
bool _stdcall opencvProcess(LPWSTR csInputPath, LPWSTR csOutputPath)
{
	...
	else 
	{
		//read hsv from file
		char hsvPath[SIZE] = "";
		sprintf(hsvPath, "%s\\InstaFilter\\adjustHSV.if", getenv("temp"));
		FILE *hsv = fopen(hsvPath, "rb");
		if(!hsv) return false;
		//read
		int randValue[3];
		char data[SIZE];
		...
		HuePosition = randValue[0];
		SaturationPosition = randValue[1];
		ValuePosition = randValue[2];
		...
		cvCreateTrackbar("色相", ctrlPanel, &HuePosition, 360, onTrackbar);
		cvCreateTrackbar("飽和度", ctrlPanel, &SaturationPosition, 200, onTrackbar);
		cvCreateTrackbar("明度", ctrlPanel, &ValuePosition, 200, onTrackbar);
