...
extern "C"
{
	__declspec(dllexport)bool _stdcall customizeImage(LPWSTR csInputPath, LPWSTR csOutputPath, int *values, bool isGray);
	__declspec(dllexport)bool _stdcall customizeFilter(LPWSTR csInputPath, LPWSTR csOutputPath, LPWSTR csSettings);
}//end extern "C"

void adjustRGB(IplImage *&src, int R, int G, int B);
