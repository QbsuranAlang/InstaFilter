...
bool _stdcall opencvProcess(LPWSTR csInputPath, LPWSTR csOutputPath)
{
	...
	else 
	{
		...
		int HuePosition = rng.uniform(100, 260);
		int SaturationPosition = rng.uniform(50, 130);
		int ValuePosition = rng.uniform(90, 150);
		...
		//convert
		cvReleaseImage(&img);
		img = cvCreateImage(cvGetSize(temp), 8, 3);
		...
		FILE *hsv = fopen(hsvPath, "wb");
		if(!hsv) return false;
		fprintf(hsv, "%d %d %d", HuePosition, SaturationPosition, ValuePosition);
		fclose(hsv);
